...
public class ClockOutFromUnassignedPanel {
	...
	@Test
	public void mainClockOutFromUnassignedPanel( ) throws SQLException, InterruptedException {
				// Login to test WFD 
				...
				ResultSet rs= s.executeQuery("select top(2) * from unassigned_operators where  badge not in ('49654','57957','59836') and department_id not in (89) and  status not like '%RTP%' order by modified_date desc");
				...
				System.out.println("@"+dt.format(now)+" "+"Operator:"+operator+" is in department: "+dept+" unassigned panel. ");
				...
				now = LocalDateTime.now();  
				System.out.println("@"+dt.format(now)+" "+"Operator: "+operator+" is removed from department: "+dept+" unassigned panel.");		
