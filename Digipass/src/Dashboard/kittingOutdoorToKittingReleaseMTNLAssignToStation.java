...
public class kittingOutdoorToKittingReleaseMTNLAssignToStation {

	public static void main(String[] args) throws InterruptedException, SQLException {
		//Before testing , Kitting dashboard should have operator in unassigned panel . 
		...
		String dept="Kitting Outdoor";
		String dest="Kitting";
		//String dest="Aluminum Coils";
		...
		ResultSet rs= s.executeQuery("SELECT TOP (1) * FROM [passport_sandbox].[dbo].[unassigned_operators ] where  department_id in  (select id from department where name ='"+dept+"') and can_work_stationIds !='' AND status NOT LIKE ('%RTP%') AND status NOT LIKE ('%MTNL%')  order by modified_date asc ");
		rs.next(); 
	   ...
	   // check db every 20 sec, wait until new clock entry created with wc= 00004  and isReleasedPool is 1.
		...
		//Go to kitting outdoor dashboard, make sure operator appeared in unassigned panel with status MTNLPEND.
		driver.findElement(By.cssSelector("i.fas.fa-arrow-left")).click();
		driver.findElement(By.xpath("//h4[contains(text(),'"+dest+"')]")).click();
		Thread.sleep(15000);
		...
		//System.out.println("operator displayed in Kitting Outdoor unassigned panel with MTNLPEND Red icon.");
		...
		//In Kitting Outdoor, Assign(drag and drop) operator to that station.
		...
		Assert.assertTrue(t);	
		now = LocalDateTime.now();  
		System.out.println("@"+dt.format(now)+" "+"dragged and dropped operator to "+dropDes+" station in "+dest+".");				
